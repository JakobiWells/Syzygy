...
function MoonCompass({ azDeg, altDeg }) {
  ...
  return (
    <svg viewBox="0 0 88 88" className="sun-widget-compass" aria-hidden="true">
      ...
      <line x1={CX} y1={CY} x2={mx} y2={my}
        stroke="#94a3b8" strokeWidth={1} strokeOpacity={below ? 0.3 : 0.5} />
      <circle cx={mx} cy={my} r={5}
        fill={below ? 'none' : '#e2e8f0'}
...
const ARC_W = 160, ARC_H = 60, HORIZON_Y = 44, ALT_SCALE = 40

function altToY(alt) { return HORIZON_Y - (alt / 90) * ALT_SCALE }

function MoonArc({ arcPts, simTime, lat, lng }) {
  ...
  const currentX = ((simTime - dayStart) / 86400000) * ARC_W

  const segments = []
  let seg = []
  for (const pt of arcPts) {
    if (pt.alt >= -1) { seg.push(pt) }
  ...
  const moonY = altToY(currentAlt)
  const above = currentAlt >= 0
  ...
  return (
    <svg viewBox={`0 0 ${ARC_W} ${ARC_H}`} className="sun-widget-arc" aria-hidden="true">
      ...
      {[0.25, 0.5, 0.75].map(f => (
        <line key={f} x1={f * ARC_W} y1={0} x2={f * ARC_W} y2={ARC_H} stroke="#e0e0dc" strokeWidth={0.5} />
      ))}
      {pathD && (
        <path d={pathD} fill="none" stroke="#94a3b8" strokeWidth={1.5} strokeLinecap="round" strokeLinejoin="round" opacity={0.8} />
      )}
      <line x1={0} y1={HORIZON_Y} x2={ARC_W} y2={HORIZON_Y} stroke="#d0d0cc" strokeWidth={1} />
      <line x1={currentX} y1={0} x2={currentX} y2={ARC_H} stroke="#1a1a1a" strokeWidth={0.75} strokeDasharray="2 2" opacity={0.4} />
      <circle
        cx={currentX} cy={above ? Math.max(3, moonY) : HORIZON_Y + 6}
        r={4}
        fill={above ? '#e2e8f0' : 'none'}
        stroke="#94a3b8" strokeWidth={1.5}
        opacity={above ? 1 : 0.35}
      />
      {above && (
        <text x={Math.min(currentX + 6, ARC_W - 22)} y={Math.max(8, Math.min(moonY - 4, HORIZON_Y - 6))}
          fontSize={6} fill="#64748b" fontFamily="inherit">
...
export default function MoonWidget({ lat, lng }) {
  ...
  const arcPts = useMemo(() => {
    ...
    return Array.from({ length: 97 }, (_, i) => {
      ...
      const p = SunCalc.getMoonPosition(t, lat, lng)
      return { frac, alt: p.altitude * 180 / Math.PI }
    })
  }, [lat, lng, Math.floor(simTime.getTime() / 86400000)])
  ...
  return (
    <div className="eclipse-sun-widget">
      <p className="eclipse-sun-label">Moon position</p>
      <div className="sun-widget-body">
        <div className="sun-widget-left">
          <MoonCompass azDeg={azDeg} altDeg={altDeg} />
          <p className="eclipse-sun-alt">
            {cardinal} · {Math.round(azDeg)}°<br />
            <span style={{ color: altDeg >= 0 ? '#475569' : '#888' }}>
              {altDeg >= 0 ? `${altDeg}° alt` : 'below horizon'}
            </span>
          </p>
        </div>
        <MoonArc arcPts={arcPts} simTime={simTime} lat={lat} lng={lng} />
      </div>
    </div>
  )
}
